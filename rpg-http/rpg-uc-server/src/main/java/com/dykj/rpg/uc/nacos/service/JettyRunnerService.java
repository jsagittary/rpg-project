...
import com.dykj.rpg.uc.nacos.config.UcConfig;
...
public class JettyRunnerService {
    ...
    public void start() {
        try {
            if (ucConfig.getUcNodes() == null) {
                logger.error("JettyRunner  start error ucNodes is null");
                System.exit(-1);
                return;
            }
            ...
            while (it.hasNext()) {
                PhysicalMachineModel physicalMachineModel = it.next();
                if (!ucConfig.getUcNodes().contains(String.valueOf(physicalMachineModel.getMachineId()))) {
                    it.remove();
                }
            }
            ...
            if (local == null) {
                logger.error("JettyRunner start error ips {} content {} ", Arrays.toString(ips.toArray()), content);
                System.exit(-1);
                return;
            }
           //jettyRunner.setHost(local.getInternetIp());
            jettyRunner.start();
    ...
    private void loadProperties() {
        ...
        try {
            ...
            String path = ucConfig.getPath() + ServerKey.UC + "/global.properties";
